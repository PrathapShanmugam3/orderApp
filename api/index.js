...
app.post('/statements/upload', checkAuth, upload.single('file'), async (req, res) => {
  try {
    ...
    // For Vercel, we'll just return empty array since pdf-parse and csv-parser
    // might have issues in serverless environment
    // The local server.js will handle the actual parsing
    console.log(`[Vercel] File upload received: ${req.file.originalname}`);
    res.json([]);
